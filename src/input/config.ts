...
async function composeProjectConfig<T extends CliFlags>(
  ...
  const coverHtmlConfig =
    config?.coverHtml === 'string'
      ? { path: config.coverHtml }
      : (config?.coverHtml as Exclude<
          VivliostyleConfigEntry['coverHtml'],
          string
        >);
  const coverHtmlImageSrc = (() => {
    const path = coverHtmlConfig?.imageSrc || otherConfig.cover;
    const absPath = path && upath.resolve(entryContextDir, path);
    if (absPath) {
      // Check file exists
  ...
  const autoGeneratedCoverHtmlPath = upath.resolve(
    workspaceDir,
    coverHtmlConfig?.path || COVER_HTML_FILENAME,
  );
  ...
  function parseEntry(entry: EntryObject): ParsedEntry {
    if (!('path' in entry)) {
      ...
      if (entry.rel === 'contents') {
        const contentsEntry: ContentsEntry = {
          rel: 'contents',
      ...
      if (entry.rel === 'cover') {
        ...
        const coverEntry: CoverEntry = {
          rel: 'cover',
          target: autoGeneratedCoverHtmlPath,
          title: entry.title ?? coverHtmlConfig?.title ?? projectTitle,
          themes,
          coverImageSrc: coverHtmlImageSrc,
          coverImageAlt: coverHtmlConfig?.imageAlt || COVER_HTML_IMAGE_ALT,
        };
        return coverEntry;
      }
    }
  ...
  if (coverHtmlImageSrc && !entries.find(({ rel }) => rel === 'cover')) {
    entries.unshift({
      rel: 'cover',
      target: autoGeneratedCoverHtmlPath,
      title: coverHtmlConfig?.title ?? projectTitle,
      themes: [...rootThemes],
      coverImageSrc: coverHtmlImageSrc,
      coverImageAlt: coverHtmlConfig?.imageAlt || COVER_HTML_IMAGE_ALT,
    });
