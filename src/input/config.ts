...
async function composeProjectConfig<T extends CliFlags>(
  ...
  function parseEntry(entry: EntryObject): ParsedEntry {
    ...
    if (isCoverEntry(entry)) {
      const coverEntry = entry as CoverEntryObject;
      const themes = entry.theme
        ? [entry.theme].flat().map((theme) =>
            parseTheme({
              theme,
        ...
        : [...rootThemes];
      themes.forEach((t) => themeIndexes.add(t));
      const coverImageSrc = ensureCoverImage(coverEntry.imageSrc || cover?.src);
      if (!coverImageSrc) {
        throw new Error(
      ...
      const parsedEntry: CoverEntry = {
        ...
        target: upath.resolve(
          workspaceDir,
          coverEntry.path || cover?.htmlPath || COVER_HTML_FILENAME,
        ),
        title: coverEntry.title ?? projectTitle,
        themes,
        coverImageSrc,
        coverImageAlt: coverEntry.imageAlt || COVER_HTML_IMAGE_ALT,
      };
      return parsedEntry;
    }
