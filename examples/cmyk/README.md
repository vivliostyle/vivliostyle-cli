...
You can output CMYK color PDFs using the `device-cmyk()` CSS function. To enable CMYK support, set `pdfPostprocess.cmyk` to a truthy value (either `true` or a configuration object) in vivliostyle.config.js. Under the hood, CMYK values cannot be represented in the browser, so Vivliostyle renders each `device-cmyk()` color as a substitute RGB color; the post-processing step then restores the original CMYK values wherever those RGB values appear in the PDF that Chromium produces. Text, borders, background colors, and SVG vector elements are the typical targets of this conversion.
...
shapes.svg, embedded below, is an SVG file designed to use C50, K50, and C50+K50, colors not used elsewhere in the CSS. The chosen RGB placeholders are `#80ffff` for C50, `#808080` for K50, and `#408080` for C50+K50. These particular values are arbitrary; Vivliostyle assigns the substitute RGB values for `device-cmyk()` colors so that they never collide with reserved entries. They are then registered in `cmyk.reserveMap` as shown in the example config, enabling CMYK colors for vector elements inside SVG.
...
The `replaceImage` array is processed front to back; the first entry that matches a given image wins. If the matched entry fails to convert the image, the image is left unreplaced; later entries are not tried. A `ReplaceFunction` may instead return `null` to decline an image; matching then continues with later entries. File-based replacement works by comparing pixel data, so it requires the original image stream to be preserved in the PDF. Simple resizing keeps the stream intact, but complex operations like filters or `object-view-box` cropping may cause the browser to rasterize the image, producing different pixel data. Images loaded from URLs also cannot be matched because no local source file exists. Additionally, images with semi-transparent pixels are not supported; only fully opaque RGB images (not RGBA) can be matched.
...
In both cases, the `builtinGrayReplacement` fallback at the end of the `replaceImage` array converts the image to grayscale.

A `ReplaceFunction` can also be used as the `replacement` in a `{ source, replacement }` entry. This is useful for images like screenshots where CMYK accuracy is not critical.
...
This CMYK feature assumes that you are aware of and in control of every colored element in your document. For complex documents, that may not always be the case. `cmyk.fallback` is a last resort for keeping the output fully CMYK: a function that receives each RGB color the color mapping does not cover (as integers on a 0-10000 scale) and returns the CMYK values to use, or `null` to leave the color unmapped.

Without it, building this example produces unmapped color warnings from the `<hr>` element in the footnote section (`#2b2b2b`, `#9a9a9a`, `#eeeeee`):

```
WARN RGB color not mapped to CMYK: {"r":1686,"g":1686,"b":1686}
WARN RGB color not mapped to CMYK: {"r":6039,"g":6039,"b":6039}
WARN RGB color not mapped to CMYK: {"r":9333,"g":9333,"b":9333}
```

The correct fix would be to style `hr` with `device-cmyk()`, but here the fallback replaces these grays through a static table, as shown in the example config. The deprecated `cmyk.overrideMap` served the same purpose; a table inside the fallback replaces it.

`builtinCmykConversion()` and `builtinGrayConversion()` are provided for automatic conversion, and `iccConversion()` converts colors through an ICC profile.

These functions make it possible to produce a CMYK PDF without any explicit `device-cmyk()` declarations, but you should not do this. What print actually requires is a PDF with an output intent (which is why PDF/X-4 can accept RGB PDFs in the first place). If you are going to run automatic CMYK conversion, you would cause fewer problems by submitting the RGB PDF directly to the print shop. Chromium's PDFs do not carry an output intent, but treating them as sRGB is good enough. In any case, `cmyk.fallback` is a tool for assisting intentional CMYK workflows, not for converting everything automatically.
...
`cmyk.mapOutput` is primarily a debugging tool. It writes the internal color mapping table to a file. Colors converted by the fallback function are not included.
