...
describe('container', () => {
  it('declares the expected entrypoint, workdir, user and locale', () => {
    const { Config } = IMAGE_INSPECT;
...
describe('document production', () => {
  ...
  describe('press-ready', () => {
    // Vivliostyle.js supplies its text-spacing filler font, which Chromium
    // embeds as a CID TrueType font with Identity-H encoding. Ghostscript
    // needs the external Identity-H CMap, normally supplied by poppler-data,
    // to interpret that encoding even though FontFile2 is embedded. Without
    // the CMap it stops at the first filler, while press-ready returns the
    // partial PDF with a successful exit status.
    it(
      'preflight rewrites the complete PDF through Ghostscript',
      async () => {
        const manuscript = path.join(DOCKER_FIXTURE_DIR, 'text-spacing.html');
        await using container = await startIdleContainer({
          bindMounts: [
            {
              source: manuscript,
              target: '/data/text-spacing.html',
              mode: 'ro',
            },
          ],
        });
        const build = await sh(
          ...
          `mkdir /data/preflight \\
            && cd /data/preflight \\
            && vivliostyle build /data/text-spacing.html --browser chrome -o source.pdf \\
            && vivliostyle build /data/text-spacing.html --browser chrome --press-ready -o out.pdf \\
            && test -s out.pdf${keepPdf('press-ready')}`,
        );
        printOnFailure(build.output);
        expect(build.exitCode).toBe(0);

        const fonts = await sh(
          container,
          'pdffonts /data/preflight/source.pdf',
        );
        printOnFailure(fonts.output);
        expect(fonts.exitCode).toBe(0);
        expect(fonts.stdout).toMatch(
          /\bCID TrueType\s+Identity-H\s+yes\s+yes\b/mv,
        );

        const sourceInfo = await sh(
          container,
          'pdfinfo /data/preflight/source.pdf',
        );
        printOnFailure(sourceInfo.output);
        expect(sourceInfo.exitCode).toBe(0);

        const outputInfo = await sh(
          container,
          'pdfinfo /data/preflight/out.pdf',
        );
        printOnFailure(outputInfo.output);
        expect(outputInfo.exitCode).toBe(0);
        expect(outputInfo.stdout).toMatch(/^Producer:.*Ghostscript/mv);

        const sourcePages = sourceInfo.stdout.match(/^Pages:\s+(\d+)$/mv)?.[1];
        const outputPages = outputInfo.stdout.match(/^Pages:\s+(\d+)$/mv)?.[1];
        expect(sourcePages).toBe('2');
        expect(outputPages).toBe(sourcePages);
      },
      Timeout.MEDIUM,
    );
  });
