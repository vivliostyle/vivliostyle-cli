...
import type { CmykConvertFunction } from '../src/config/replace-image.js';
...
describe('composeColorConverters', () => {
  it('tries converters in order until one returns a value', async () => {
    const first = vi.fn<CmykConvertFunction>().mockReturnValue(null);
    const second = vi
  ...
  it('caches the result per color', async () => {
    const fn = vi
      .fn<CmykConvertFunction>()
      .mockReturnValue({ c: 0, m: 0, y: 0, k: 10000 });
    const miss = vi.fn<CmykConvertFunction>().mockReturnValue(null);
...
describe('guardConvertFunction', () => {
  ...
  it('passes through a null result without warning', async () => {
    const spy = vi.spyOn(Logger, 'logWarn').mockImplementation(() => {});
    const convert = guardConvertFunction(() => null);
  ...
  it('returns null and warns once per distinct error when the function throws', async () => {
    ...
    expect(spy).toHaveBeenCalledWith(
      expect.stringContaining('Failed to apply fallback conversion'),
    );
    spy.mockRestore();
  });

  it('rejects results that would not pass the CMYK value schema', async () => {
    const spy = vi.spyOn(Logger, 'logWarn').mockImplementation(() => {});
    const invalidResults = [
      { c: Number.NaN, m: 0, y: 0, k: 0 },
      { c: 0, m: 0, y: 0, k: Number.POSITIVE_INFINITY },
      { c: 0.5, m: 0, y: 0, k: 1 },
      { c: 0, m: 0, y: 0, k: 10001 },
      { c: -1, m: 0, y: 0, k: 0 },
    ];
    ...
    for (const result of invalidResults) {
      const convert = guardConvertFunction(() => result);
      expect(await convert({ r: 1, g: 2, b: 3 })).toBeNull();
    }
    expect(spy).toHaveBeenCalledTimes(invalidResults.length);
    expect(spy).toHaveBeenCalledWith(
      expect.stringContaining('Invalid fallback conversion result'),
    );
    spy.mockRestore();
  });

  it('warns once per distinct invalid result', async () => {
    const spy = vi.spyOn(Logger, 'logWarn').mockImplementation(() => {});
    const convert = guardConvertFunction(() => ({ c: 0.5, m: 0, y: 0, k: 1 }));
