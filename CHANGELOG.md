...
- Report a conflicting version warning for CSS `@import` rules of the same theme package only when the requested version ranges do not intersect. Imports without a version specifier (e.g. `@import '@vivliostyle/theme-base/page'`) and compatible ranges now follow the version requested first without a warning. ([#924](https://github.com/vivliostyle/vivliostyle-cli/pull/924))
...
- Resolve CSS `@import` rules referring to the package containing the importing stylesheet by its own name, like the self-referencing of Node.js, instead of installing the package with the same name from npm. ([#919](https://github.com/vivliostyle/vivliostyle-cli/pull/919))
...
- Run tests with Vitest's default `forks` pool on all platforms to avoid native-module segfaults in CI. ([#816](https://github.com/vivliostyle/vivliostyle-cli/pull/816))

- Update Vivliostyle.js to 2.43.1: Bug Fixes ([#818](https://github.com/vivliostyle/vivliostyle-cli/pull/818))
