...
`npm create book@latest` でも同様に指定できます：
...
ビルトインテンプレートはすべてこのマニフェストを持っています。ビルトインテンプレートは Vivliostyle CLI リポジトリの `main` ブランチから取得されるため、実行中の CLI より新しいバージョンを要求することがあります。その場合、CLI は処理を中断せず、警告を表示したうえで自身のバージョンに対応するリリースのテンプレート（例えば CLI 11.4.0 では `gh:vivliostyle/vivliostyle-cli/templates/basic#v11.4.0`）を代わりに使用します。最新のテンプレートを使用するには `@vivliostyle/cli` を更新してください。
